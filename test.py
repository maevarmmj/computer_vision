...
def get_HSV_value(event, x, y, flags, param):
     if event == cv.EVENT_LBUTTONDOWN:  # Clic gauche
         pixel = hsv[y, x]  # (H, S, V)
         print(f"HSV: {pixel}")
...
cap = cv.VideoCapture('Video/tennis_tab.mp4') ##VIdeo tableau cadrillage car sans find corner ne fonctionnais pas bien
frameTime = 200
trajectoire = []
...
while cap.isOpened():
    ...
    cv.setMouseCallback("frame", get_HSV_value) ##envoie appelle la fonction pour afficher la couleur
